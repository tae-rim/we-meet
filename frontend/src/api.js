...
// 요청을 보내기 전에 가로채서 토큰을 헤더에 심는 코드 (Interceptor)
apiClient.interceptors.request.use(
  (config) => {
    // 로컬 스토리지에서 토큰을 꺼냅니다.
    const token = localStorage.getItem("access_token");
    
    // 토큰이 있으면 헤더에 'Bearer 토큰값' 형태로 붙임
    if (token) {
  ...
  const response = await apiClient.post("/api/analysis/", formData, {
    ...
    onUploadProgress: (progressEvent) => {
      if (progressEvent.total) {
        ...
        if (onProgress) {
            // 99%까지만 보여주고, 100%는 응답이 왔을 때 찍도록 함
...
export const loginUser = async (email, password) => {
  // 중요: FastAPI OAuth2PasswordRequestForm은 'form-data'를 요구
  // JSON이 아닌 URLSearchParams를 사용
  const formData = new URLSearchParams();
  formData.append('username', email); // 백엔드는 'username'이라는 필드명을 기다림 (값은 이메일)
...
export const registerUser = async (email, username, password) => {
  // Pydantic 스키마(UserCreate)와 필드명이 정확히 일치해야 함.
